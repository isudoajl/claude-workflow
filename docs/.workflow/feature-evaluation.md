...
| D7: Timing | 3 | No blocking prerequisites. Project is stable. But the recent consolidation of commands (workflow-feature to workflow-new-feature, workflow-improve-functionality to workflow-improve) suggests the focus should remain on stabilizing existing commands, not adding a meta-routing layer. |
...
2. **14 command files** with explicit descriptions in `core/commands/` -- each with YAML frontmatter `description` that Claude uses for matching (verified in `workflow-bugfix.md`, `workflow-improve.md`, `workflow-new-feature.md`, `workflow-diagnose.md`, etc.).
...
2. **Silent failure is worse than explicit choice**: When a user types `/workflow:bugfix`, they know what pipeline they are entering. If a dispatcher silently chooses the wrong pipeline, the user may not notice until several agents deep -- by which point significant context and time have been consumed.
